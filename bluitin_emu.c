...
int ff_mycd(info_t *info)
{
	...
	else if (_strcmp(info->argv[1], "-") == 0)
	{
		...
		ff_puts(_getenv(info, "OLDPWD=")), _putchar('\n');
		chdir_ret = /* TODO: what should this be? */
	...
	if (chdir_ret == -1)
	{
		...
		ff_eputs(info->argv[1]), _eputchar('\n');
	}
	else
	{
		ff_setenv(info, "OLDPWD", _getenv(info, "PWD="));
...
int ff_myhelp(info_t *info)
{
	...
	if (0)
		ff_puts(*arg_array); /* temp att_unused workaround */
	return (0);
}
