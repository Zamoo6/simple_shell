...
/**
 * _getvvenv - gets the value of an environ variable
 * @info: Structure containing potential arguments. Used to maintain
 * @name: env var name
 *
 * Return: the value
 */
char *_getvvenv(info_t *info, const char *name)
{
...
int _oursetenv(info_t *info)
{
	...
	if (_setenv_rv(info, info->argv[1], info->argv[2]))
		return (0);
	return (1);
}
...
int _ourunsetenv(info_t *info)
{
	...
	for (i = 1; i <= info->argc; i++)
		_unsetenv_rv(info, info->argv[i]);

	return (0);
}

/**
 * env_populate_lnls - populates env linked list
 * @info: Structure containing potential arguments. Used to maintain
 *          constant function prototype.
 * Return: Always 0
 */
int env_populate_lnls(info_t *info)
{
	list_t *node = NULL;
	size_t i;
